[JOURNAL (10).md](https://github.com/user-attachments/files/28892697/JOURNAL.10.md)
<!--
  This journal is auto generated by Stasis.
-->

# 6/12/2026 6 PM - CAD Update (Again)

_Time spent: 0.5h_

# Overview
After recieving feedback for the project again, i fixed the minor issues that it had
I extruded a round hole around the joystick bottom that would allow it to move freely.
Also, the spaces for the buttons where corrected and the old 3d models were deleted

![Screenshot 2026-06-12 181327](https://stasis.hackclub-assets.com/images/1781288469606-utjwmr.png)

![Screenshot 2026-06-12 181524](https://stasis.hackclub-assets.com/images/1781288472932-4htws9.png)

![Screenshot 2026-06-12 181545](https://stasis.hackclub-assets.com/images/1781288475935-2ltqsx.png)

![Screenshot 2026-06-12 181606](https://stasis.hackclub-assets.com/images/1781288496217-gdze4k.png)

![Screenshot 2026-06-12 181627](https://stasis.hackclub-assets.com/images/1781288503250-x4zzk3.png)

These images show that the bottons do have a space to stay and can be wired. Since the pins on the button are slightly angled, itll fit well and hold tightly in the space created for them, a little hot glue can also be used to hold them in place while still allowing them to be removeable when needed



# 5/9/2026 7 AM - CAD Update 2

_Time spent: 2h_

# Overview
After the second set of feedback i finally realized that there is are a bit more problems with the case design. I choose to redesign the whole enclosure because the former design seemed Unnecessarily complicated (The handles).
The new version is a simple enclosure that has holes cut out at the right sizes for the components (hopefuly, cause i took my time to measure and sketch accordingly)
I added screws for the case as well, and supports for the components inside

![Screenshot 2026-05-09 073650](https://stasis.hackclub-assets.com/images/1778312917317-ipsx45.png)

![Screenshot 2026-05-09 080750](https://stasis.hackclub-assets.com/images/1778312942337-to53jb.png)

![Screenshot 2026-05-09 080838](https://stasis.hackclub-assets.com/images/1778312958252-rdjzie.png)

![Screenshot 2026-05-09 074050](https://stasis.hackclub-assets.com/images/1778312972686-ttlcpf.png)

I think the case is okay this time around



# 4/29/2026 8 PM - CAD polish

_Time spent: 1h_

# Overview
After I recieved the feedback from the reviewer, I had to go improve the CAD design for this project case.
The issue the review addressed was not being able to assemble, which is true after i re inspected it. So i modified the project by extending the side gap edges downward so it can act as a sort of protrusion that the bottom case can slide on and attach to tightly.

![Screenshot 2026-04-29 210819](https://stasis.hackclub-assets.com/images/1777494922112-5w0qum.png)

I also took the chance to try a couple things and i figured out i could merge the bodies that made the top part of the case into one making for a cleaner finish.

![Screenshot 2026-04-29 210454](https://stasis.hackclub-assets.com/images/1777495104312-ydnp7k.png)

I genuienly enjoy fixing small problems like this especially from the reviewers (NOTTTTT saying i want my project rejected multiple times thooo haha).
If only the reviews could move along faster, but hey, theyre probably trying their best too.
Anyways ive fixed that issue and i dont see any clear issues anymore so... hopefully its approved this time :p



# 4/18/2026 6 PM - CAD Design

_Time spent: 3h_

# Overview
I now had a clear parts list and wiring diagram, the only thing left was the building the 3D model

I used a lot of new features in fusion for this project. It was genuinely very exiciting, especially because i figured out how to use some very helpful features myself without a tutorial.

All the 3D models for the parts i used were gotten from GrabCAD, although getting the exact 3D models for the parts i used was kinda annoying, but i got them anyways.

Even though this has been my most complicated CAD build so far, it was still the most enjoyable

![3D Model 2](https://stasis.hackclub-assets.com/images/1776537311990-3q5ngd.png)

Its designed in a sort of console design, that would make it easier to hold.
And the screen is slightly angled so its more easily visible when placed on a flat surface

This the top view without the top cover

![CMP image 1](https://stasis.hackclub-assets.com/images/1776537536059-ggh7i5.png)

This is the view from the bottom with the underside of the case removed

![CMP image 2](https://stasis.hackclub-assets.com/images/1776537606265-3ks3ge.png)

And thats it!



# 4/18/2026 6 PM - Wiring Diagram and Unit Testing

_Time spent: 5h_

# Overview
Since i had decided what parts are going to be in my project, i started working on the wiring diagram.
Even though this is my second submitted project here, it was actually my first attempt at creating a wiring circuit, so as you would imagine there were a lot of mistakes.

Although i made quite a funny mistake by putting all four pins of my joystick on a single column, had a headache trying to figure out why my joystick is mixing signals. I eventually went on a break, took a walk, and mid walk genuinely realized what i had done.

I also had issues with the buttons, cause i didnt understand which sides of the four pins were internally attached, so it didnt work for quite a while before i realized.

I tested each part individually to make sure everything was wired properly.
After i had tested everything, the encoder just randomly started registering double signals, but after learning about bounce and stuff, i was able to fix it

![CAD MacroPad Prototype 1](https://stasis.hackclub-assets.com/images/1776536126907-z7aubl.png)

After everything i went through in this stage i genuinely believed that this project could reach tier 3, but unfortunately i didnt meet the time requirement, and further thought made me realize its still only tier 2. Almost tier 3 though... i think



# 4/18/2026 6 PM - Material Sourcing and BOM creation

_Time spent: 1h_

# Overview
In this session, I started by thinking up a suitable project. It didnt take that long for me to decide on this, well, mostly because its complexity is manageable for my level... for now...
It also addresses an issue i have with working with CAD software so its a win win situation.

The project will make use of the following:

### Raspberry pi PICO RP2040

![RASPBERRY PI](https://stasis.hackclub-assets.com/images/1776534806992-wemdos.png)

### OLED Display

![oled](https://stasis.hackclub-assets.com/images/1776534841668-nlq53y.png)

### Rotary Encoder

![enc](https://stasis.hackclub-assets.com/images/1776534937061-gyxy3q.png)

### Joystick Module

![JOYSTICK MODULE](https://stasis.hackclub-assets.com/images/1776534977262-3wuhfl.png)

### Buzzer

![buzz](https://stasis.hackclub-assets.com/images/1776535052324-nq854w.png)

### 800 Point Breadboard

![breadboard](https://stasis.hackclub-assets.com/images/1776535105889-afylhv.png)

- Jumper wires and push buttons are a part of this project but arent included in this BOM, since a previous project has covered those
